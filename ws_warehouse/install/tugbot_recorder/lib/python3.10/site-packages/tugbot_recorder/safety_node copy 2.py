...
# Hard stop (any sector)
D_STOP = 0.30

# Front warn thresholds
D_WARN_F  = 1.0
D_WARN_FL = 1.0
D_WARN_FR = 1.0
...
# NEW: dedicated very-close side stop threshold
D_STOP_SIDE = 0.40
...
# ---- TTC + EVADE ----
TTC_SIDE_THRESH = 1.5      # seconds
TTC_EPS_DDOT    = 0.20     # m/s
D_SIDE_TTC_MAX  = 3.0      # only compute TTC when side object is within this range

# NEW: TTC stability gates
TTC_HIT_COUNT_REQ = 3      # require N consecutive TTC hits before EVADE
V_TTC_ENABLE_MIN  = 0.05   # only allow TTC-based EVADE when robot is actually moving

T_EVADE = 0.3              # seconds
D_CLEAR_FRONT = 1.5        # if front is very clear -> speed up to clear intersection
V_EVADE_BACK = 0.30        # m/s (reverse)
V_EVADE_FWD  = 0.50        # cap to robot max speed (policy cap)
W_EVADE = 0.0
# ==============================================================================
...
class SafetyFSMNode(Node):
    def __init__(self):
        ...
        # Cached values for status prints
        self.last_dR = 999.0
        self.last_dFR = 999.0
        self.last_dF = 999.0
        self.last_dFL = 999.0
        ...
        # NEW: persistence counter for TTC-based evade
        self.ttc_hit_count = 0
    ...
    def status_loop(self):
        ...
        self.get_logger().info(
            ...
            f"ttc_side={ttc_disp:.2f}s hits={self.ttc_hit_count} | "
            f"cmd_in v={cmd.linear.x:.2f} w={cmd.angular.z:.2f}"
        )
    ...
    def _publish_viz(self, dR, dFR, dF, dFL, dL, ttc_side):
        if self.state == State.NAVIGATE:
            ...
            safety_col = self._c(0.7, 0.2, 1.0, 1.0)
        ...
        mission_col = self._c(0.0, 1.0, 0.0, 1.0) if self.mission_status == "GOAL_REACHED" else self._c(1.0, 0.55, 0.0, 1.0)
        bg = self._c(0.0, 0.0, 0.0, 0.35)

        self.pub_overlay_safety.publish(self._overlay(f"● SAFETY: {self.state.name}", safety_col, bg, x=10, y=10))
        self.pub_overlay_mission.publish(self._overlay(f"● MISSION: {self.mission_status}", mission_col, bg, x=10, y=75))
        ...
        ttc_show = (ttc_side if ttc_side != float("inf") else 999.0)
        self._publish_text_marker(5, math.radians(+90), r_lbl, f"TTC:{ttc_show:.2f}s n={self.ttc_hit_count}")
    ...
    def control_loop(self):
        ...
        # Hard stop if anything very close (global)
        stop_hit = (min(dF, dFL, dFR, dL, dR) <= D_STOP)

        # Additional "very close side" stop
        side_stop_hit = (side_min <= D_STOP_SIDE)

        # Side warn by distance
        side_near = (side_min < D_WARN_SIDE)

        # Front warn by distance
        front_near = (dF < D_WARN_F) or (dFL < D_WARN_FL) or (dFR < D_WARN_FR)

        # TTC hit (gated)
        moving_enough = abs(cmd_in.linear.x) > V_TTC_ENABLE_MIN
        ttc_valid_range = (side_min < D_SIDE_TTC_MAX)
        ttc_hit_raw = (ttc_side < TTC_SIDE_THRESH) and (mission_status != "GOAL_REACHED")
        ttc_hit = ttc_hit_raw and moving_enough and ttc_valid_range and side_near

        # Maintain persistence counter
        if ttc_hit:
            self.ttc_hit_count = min(self.ttc_hit_count + 1, TTC_HIT_COUNT_REQ + 2)
        else:
            self.ttc_hit_count = max(self.ttc_hit_count - 1, 0)

        ttc_persistent = (self.ttc_hit_count >= TTC_HIT_COUNT_REQ)

        # --- Transitions (priority order) ---
        if stop_hit or side_stop_hit:
            why = "min(d*) <= D_STOP" if stop_hit else f"side_min={side_min:.2f} <= D_STOP_SIDE"
            self._set_state(State.STOP_EMERGENCY, why)

        else:
            # Corridor damping (optional)
            ...
            # TTC-based EVADE (only when persistent)
            if ttc_persistent and self.state not in (State.STOP_EMERGENCY, State.WAIT):
                self._set_state(State.EVADE_SIDE, f"TTC side={ttc_side:.2f}s < {TTC_SIDE_THRESH:.2f}s (persist)")
            ...
            if self.state in (State.NAVIGATE, State.SLOW, State.CORRIDOR_DAMPING):
                if front_near or side_near:
                    self._set_state(
                        State.SLOW,
                        f"warn: dF={dF:.2f} dFL={dFL:.2f} dFR={dFR:.2f} dL={dL:.2f} dR={dR:.2f}"
                    )
                else:
        ...
        if self.state == State.NAVIGATE:
            ...
            if front_clear_for_burst:
                out.linear.x = clamp(+V_EVADE_FWD, -V_MAX_POLICY, V_MAX_POLICY)
            else:
                out.linear.x = clamp(-V_EVADE_BACK, -V_MAX_POLICY, V_MAX_POLICY)
