...
# ===================== USER SETTINGS =====================
RUN_FOLDER = "Sr02_gA+B_eps15_IMG1_LID1_STA1_CFC64_OH1"  # <- your run folder
...
def build_sequence_model(IMG_SHAPE, LIDAR_DIM,
    ...
    if use_state:
        ...
        # Only slice/concat goal when training actually used goal onehot AND it has >1 dim
...
class InferenceNode(Node):
    def __init__(self):
        ...
        #-------------------------------------------publishers-------------------------------------------
        #self.twist_pub = self.create_publisher(Twist, "/model/tugbot/cmd_vel", 10) # choose it for only lnn prediction node

        self.twist_pub = self.create_publisher(Twist, "/pred_vel", 10)             # choose it for pred and safety node
        #--------------------------------------------------------------------------------------
        ...
        self.goal2idx = goals_md.get("goal2idx", {})
        ...
        self.use_goal_onehot = bool(exp.get("use_goal_onehot", False))  # IMPORTANT
        ...
        # goal ids (may be ["C"] in your single-goal run)
        self.goal_ids = goals_md.get("ids", [])
        self.N_GOALS = int(hp.get("N_GOALS", len(self.goal_ids) if self.goal_ids else 1))

        # STATE_DIM must match training
        self.STATE_DIM = int(hp.get("STATE_DIM", self.STATE_BASE_DIM + (self.N_GOALS if self.use_goal_onehot else 0)))

        # Only require GOAL_ID membership when using onehot with >1 goals
        if self.use_goal_onehot and (self.N_GOALS > 1):
        ...
        self.log_w.writerow([
            "t_unix", "t_ros_ns", "run_folder", "run_id", "goal_id",
            "v_pred_lin", "w_pred_ang", "v_cmd_lin", "w_cmd_ang",
            "x", "y", "yaw", "dist_to_goal",
        ])
        self.log_f.flush()
        ...
        self.get_logger().info(
            ...
            f"use_goal_onehot={self.use_goal_onehot} N_GOALS={self.N_GOALS} STATE_DIM={self.STATE_DIM} "
    ...
    def _load_model(self):
        if os.path.exists(self.model_path):
            try:
                ...
                m = tf.keras.models.load_model(self.model_path, compile=False)
                self.get_logger().info("[MODEL] load_model OK")
                return m
            except Exception as e:
                self.get_logger().warning(f"[MODEL] load_model failed, fallback to weights. Reason: {e}")
        ...
        model = build_sequence_model(
            ...
            use_goal_onehot=self.use_goal_onehot,
            rnn_backend=self.rnn_backend,
        )
        print(f"rnn_backend={self.rnn_backend}")
    ...
    def odom_callback(self, msg: Odometry):
        try:
            if getattr(self, "odom_frame", None) is None:
                self.odom_frame = msg.header.frame_id  # odom pose frame 

            if not getattr(self, "goal_published", False):
    ...
    def publish_goal_pose(self):
        gmsg = PoseStamped()
        gmsg.header.stamp = self.get_clock().now().to_msg()
        gmsg.header.frame_id = getattr(self, "odom_frame", "odom")  # same as odom pose frame [web:94]
        gmsg.pose.position.x = self.goal_pose_world[0]
        gmsg.pose.position.y = self.goal_pose_world[1]
        gmsg.pose.position.z = 0.0
        # If you don’t need yaw for “goal reached”, orientation can be identity:
        gmsg.pose.orientation.w = 1.0
        self.goal_pub.publish(gmsg)
    ...
    def _build_state_z(self, robot_pose_world, goal_pose_world):
        ...
        # Match training: if onehot was not used, state is only 5-dim
        if self.use_goal_onehot and (self.N_GOALS > 1):
            if GOAL_ID not in self.goal2idx:
                raise ValueError(f"GOAL_ID='{GOAL_ID}' not in goal2idx keys={list(self.goal2idx.keys())}")
            one_hot = np.zeros(self.N_GOALS, dtype=np.float32)
            one_hot[int(self.goal2idx[GOAL_ID])] = 1.0
            state = np.concatenate([base_z, one_hot], axis=0).astype(np.float32)
        else:
            state = base_z.astype(np.float32)
    ...
    def _input_unpack_live(self, img_seq, lidar_seq, state_seq):
        ...
        if self.use_state:
            ...
            # goal-only mode (rare): provide goal_vec
            if self.use_goal_onehot and (self.N_GOALS > 1):
                goal_vec = state_seq[:, 0, self.STATE_BASE_DIM:]  # (B,n_goals)
            else:
                goal_vec = np.ones((state_seq.shape[0], 1), dtype=np.float32)
            xs += (goal_vec,)
    ...
    def inference_step(self):
        ...
        try:
            ...
            # publish
            tw = Twist()
            # Use quantized, or raw clipped (your choice)
            tw.linear.x  = v_cmd  #float(np.clip(v_pred, -0.5, 0.5))
            tw.angular.z = w_pred  #float(np.clip(w_pred, -0.5, 0.5))
            ...
            self.log_w.writerow([
                t_unix, t_ros_ns, RUN_FOLDER, self.run_id, GOAL_ID,
                v_pred, w_pred, v_cmd, w_cmd,
                x, y, yaw, self.dist_to_goal,
            ])
