...
W_CORRIDOR = 0.0
V_CORRIDOR_SCALE = 0.85

D_STOP = 0.30
D_STOP_GOAL_FRONT = 0.60   # stop if close-in-front while at goal
...
D_WARN_SIDE = 1 
D_WARN_SIDE_TTC = 1.5    # for ttc based gate
...
class SafetyFSMNode(Node):
    ...
    def status_loop(self):
        ...
        dist_disp = self.last_goal_dist if (self.last_goal_dist is not None) else float("nan")
        self.get_logger().info(
            ...
            f"goal_dist={dist_disp:.2f} valid={int(self.last_goal_valid)} "
            f"frames(goal='{self.last_goal_frame}', odom='{self.last_odom_frame}') | "
            f"cmd_in v={cmd.linear.x:.2f} w={cmd.angular.z:.2f}"
        )
    ...
    def control_loop(self):
        ...
        if (self.goal_pose is not None) and (self.odom_pose is not None):
            if self.goal_pose.header.frame_id != self.odom_pose.header.frame_id:
                ...
                self.last_goal_valid = goal_valid
                self.last_goal_frame = self.goal_pose.header.frame_id if self.goal_pose is not None else ""
                self.last_odom_frame = self.odom_pose.header.frame_id if self.odom_pose is not None else ""
        ...
        side_min = min(dL, dR)
        front_min = min(dF, dFL, dFR)
        goal_front_stop_hit = (mission_status == "GOAL_REACHED") and (front_min <= D_STOP_GOAL_FRONT)

        side_ttc_hit = (
            (ttc_side < TTC_SIDE_THRESH) and
            (mission_status != "GOAL_REACHED") and
            (side_min < D_WARN_SIDE_TTC)
        )


        # Priority: hard stop first, then TTC evade (you can swap if you prefer)
        # Terminal latch: once finished, never leave (until you explicitly reset on new goal)
        if self.state == State.MISSION_FINISHED:
            pass

        elif goal_front_stop_hit:
            self._set_state(
                State.MISSION_FINISHED,
                f"finish: GOAL_REACHED and front_min={front_min:.2f} <= {D_STOP_GOAL_FRONT:.2f}"
            )
            ...
            # TTC-based side evade (blocked at GOAL_REACHED already by side_ttc_hit definition)
        ...
        elif self.state == State.MISSION_FINISHED:
            out.linear.x = 0.0
            out.angular.z = 0.0
