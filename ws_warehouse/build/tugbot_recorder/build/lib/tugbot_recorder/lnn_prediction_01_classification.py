...
# ===================== USER SETTINGS =====================
RUN_FOLDER = "CLS_IMG1_LID1_STA1_CFC64_GC"   # <-- set to your classification run folder
...
ACTIONS_3 = np.array([-0.5, 0.0, 0.5], dtype=np.float32)  # class 0,1,2 -> value
...
# ---------------- Model blocks (classification head) ----------------
...
def build_sequence_model_classif(IMG_SHAPE, LIDAR_DIM,
                                 STATE_DIM, STATE_BASE_DIM, SEQ_LEN,
                                 CFC1_UNITS,
    ...
    # 2 outputs * 3 classes = 6 logits per timestep
    logits6 = layers.TimeDistributed(layers.Dense(6, dtype="float32"), name="logits6")(x)
    logits  = layers.Reshape((-1, 2, 3), name="logits")(logits6)  # (B,T,2,3)
    return models.Model(inputs=inputs, outputs=logits, name="LNN_modal_classif")


# ---------------- ROS Node ----------------
class InferenceNode(Node):
    def __init__(self):
        super().__init__("model_inference_classif")
        ...
        self.STATE_DIM = int(hp["STATE_DIM"])
        ...
        self.get_logger().info(
            f"[RUN] {RUN_FOLDER} | classification | seq_len={self.seq_len} | state_dim={self.STATE_DIM}"
        )

    def _load_model(self):
        if os.path.exists(self.model_path):
            try:
                self.get_logger().info(f"[MODEL] load_model: {self.model_path}")
        ...
        self.get_logger().info("[MODEL] Rebuild + load_weights fallback")

        model = build_sequence_model_classif(
    ...
    def _build_state_z(self, robot_pose_world, goal_pose_world):
        ...
        return base_z.astype(np.float32)  # (5,)
    ...
    def inference_step(self):
        ...
        try:
            ...
            state_seq = np.stack(self.buf_state, axis=0)[None, ...]  # (1,T,5)

            x_in = self._input_unpack_live(img_seq, lidar_seq, state_seq)

            logits = self.model(x_in, training=False).numpy()  # (1,T,2,3) expected
            # pick last timestep classes
            cls_lin = int(np.argmax(logits[0, -1, 0, :]))
            cls_ang = int(np.argmax(logits[0, -1, 1, :]))

            v_cmd = float(ACTIONS_3[cls_lin])
            w_cmd = float(ACTIONS_3[cls_ang])
            ...
            # publish debug vector: [cls_lin, cls_ang, v_cmd, w_cmd]
            msg = Float32MultiArray()
            msg.data = [float(cls_lin), float(cls_ang), v_cmd, w_cmd]
            ...
            self._log_ctr += 1
            if self._log_ctr % max(1, int(FPS)) == 0:
                self.get_logger().info(
                    f"CLS lin={cls_lin} ang={cls_ang} | CMD v={v_cmd:.2f} w={w_cmd:.2f} | dist={self.dist_to_goal:.2f}m"
                )
