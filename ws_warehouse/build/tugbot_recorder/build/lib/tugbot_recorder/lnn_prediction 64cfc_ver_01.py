...
""" cfc 64  32 seq one goal change goals name - first code for prediction  """
...
GOAL_ID    = 'B'      # <-- choose: 'Home' / 'A' / 'B' / 'C'
SEQ_LEN    = 32
FPS        = 10.0     # target inference rate (Hz) once buffers are full

# Model run selection (mirrors your training script structure)
#RUN_FOLDER     = "home2A_seq32_cfc64_tr_02_cmd_vel_5"    # <-- change this per run
#RUN_FOLDER     = "home2A_seq32_cfc64_tr_03_state_4"
RUN_FOLDER     = "home2A_seq32_cfc64_vel_5_goals_3_st_04_tr_01"
...
class InferenceNode(Node):
    ...
    def _build_state_z(self, robot_pose_world, goal_pose_world):
        ...
        if self.state_dim == 11:
            ...
            # Safety: if you later experiment with STATE_DIM=6, etc.
            raise ValueError(f"Unsupported STATE_DIM={self.state_dim} in run_info.json")
    ...
    def inference_step(self):
        ...
        try:
            ...
            """    ------------------------------------------old code for SCALING v lin ------------------------------------------

            pred = self.model.predict([img_seq, lidar_seq, state_seq], verbose=0)
            raw_v, raw_w = float(pred[0, -1, 0]), float(pred[0, -1, 1])

            # Scale forward velocity to max 0.5 m/s
            SCALE_V = 0.5
            v_lin = raw_v * SCALE_V

            # (optional) keep angular velocity as-is for now
            v_ang = raw_w

            ------------------------------------------new code for SCALING v lin ------------------------------------------
            """
